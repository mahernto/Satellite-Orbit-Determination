...
if (t==tout)    % No integration
      disp('[INFO] t == tout, no integration needed.');
    return;
end
...
if ( ( relerr <  0.0         ) || ...            % Negative relative error bound
     ...
     ( (State_ ~= DE_STATE.DE_INIT) &&       ...
     (t ~= told)           ) )
     disp('[ERROR] Invalid error tolerances.');
...
while (true)   % Start step loop
  ...
  if (abs(x-t) >= absdel)
       disp('[STEP] Interpolating final result at tout.');  disp(tout);
      yout  = zeros(n_eqn,1);
      ypout = zeros(n_eqn,1);
      g(2)   = 1.0;
      rho(2) = 1.0;
  ...
  if ( ~PermitTOUT && ( abs(tout-x) < fouru*abs(x) ) )
      ...
      told      = t;                % Store independent variable
      OldPermit = PermitTOUT;
       disp('[INFO] Extrapolated final step to tout.');
      return;                       % Normal exit
  end
...
%   end
  ...
  h  = sign_(min(abs(h), abs(tend-x)), h);
    disp('[MATLAB] yy:');   disp(yy);
 disp('[MATLAB] wt:');  disp(wt);
  for l=1:n_eqn
