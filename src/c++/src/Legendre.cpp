/// @file Legendre.cpp
/// @brief Source file for Legendre function
/// @date 2025-04-10
...
void Legendre(double n, double m, double fi, Matrix &pnm, Matrix &dpnm){
    ...
    while(true){
        for (int i = k; i <= n; i++){
            pnm(i+1,j+1)=sqrt((2.0*i+1)/((i-j)*(i+j)))*((sqrt(2*i-1)*sin(fi)*pnm(i,j+1))-(sqrt(((i+j-1.0)*(i-j-1.0))/(2*i-3))*pnm(i-1,j+1)));
        }
        j = j+1;
        k = k+1;
        if (j>m)
    ...
    while(true){
        for (int i = k; i <= n; i++){
            dpnm(i+1,j+1)=sqrt((2.0*i+1)/((i-j)*(i+j)))*((sqrt(2*i-1)*sin(fi)*dpnm(i,j+1))+(sqrt(2*i-1)*cos(fi)*pnm(i,j+1))-(sqrt(((i+j-1.0)*(i-j-1.0))/(2.0*i-3.0))*dpnm(i-1,j+1)));
        }
        j = j+1;
        k = k+1;
        if (j>m)
